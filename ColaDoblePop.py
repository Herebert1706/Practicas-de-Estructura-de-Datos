...
#Creo mi contadot en 0 que es para saber cuantos datos estan dentro de mi cola
cont = 0
...
#El else que se aplica es para que me diga si la cola ya se encuentra llena que eso es cuando ya se agregaron 10 datos
		...
		if cont == 10:
			...
			if last_front == 0:
				...
				last_front -= 1
				cont += 1
#Declaro mi metodo para agregar datos desde la parte trasera de mi cola, las instrucciones son muy parecidas a la del push frontal entonces este lo que hace se va			return Push_front(Cola_,last_front, last_back,cont)
	...
	if Cola_[last_back] == 0:
		...
		if cont == 9:
			...
			if last_back == 9:
				...
				cont += 1
				return Push_last(Cola_,last_front, last_back,cont)
#Creo mi metodo pop para poder eliminar un elemento desde la parte frontal de la cola, para que esto se pueda realiar se hacer unas indicaciones diciendo que si hay un elemento se puede eliminar
#Si no pues se pasa al siguiente campo y si no hay nada pues no podra eliminar algo
def Pop_front(Cola_,first_front,first_back,cont):
 	if Cola_[first_front-1] == 0:
 		...
 		first_front = 10
 		return Cola_,first_front,cont
#Hago mi metodo para eliminar desde el ultimo elemento de la lista hasta adelanta, son las mismas instrucciones que el pop_front pero invertidas y con un ligero cambio
